...
移植来源：opensoul/src/cortex/loop_guard.py（同一100-agent调研产出的五源合抄实现：
ag2 LoopDetector + goose RepetitionInspector + Khoj组合签名 + anything-llm
3-gram Jaccard + DeerFlow三级渐进）。acp-proxy是独立进程/venv，无法import
opensoul包，按既有先例（tool_output_handler.py）在agent/下持有一份。

接线位置：agent/soulmate_agent.py _run_llm_with_tools 真实ACP工具循环
（:8092 ws→SoulMateAgent，OpenMate聊天页实际使用路径）。
接线侧差异：agent构造guard时cooldown_seconds=0（疲劳抑制由wiring级升级链
承担：第2次INTERVENE→FORCE_STOP），详见SoulMateAgent._loop_guard_for。
