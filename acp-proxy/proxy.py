...
class ACPProcess:
    ...
    async def start(self):
        ...
        # Create default session so send_message works without explicit session_id
    ...
    async def send_message(self, text: str, session_id: str | None = None) -> dict[str, Any]:
        if not self.is_running or not self._initialized:
            await self.start()
        # Use default session if no valid session_id provided
        sid = session_id or self._default_session_id or "default"
        # Validate session_id looks like a UUID (not a random string like "test")
