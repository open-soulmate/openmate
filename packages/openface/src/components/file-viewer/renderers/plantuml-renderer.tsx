 ...
 *
 * 功能：
 * - 动态加载 viz-global.js（Graphviz 布局引擎，SSR 兼容）
 * - 动态导入 @plantuml/core
 * - 支持 .puml / .plantuml 文件
...
/* ------------------------------------------------------------------ */
/*  动态加载 viz-global.js（Graphviz 布局引擎）                         */
/* ------------------------------------------------------------------ */

/** 标记 viz-global.js 是否已加载完成 */
let vizGlobalLoaded = false;
/** 加载中的 Promise（防止重复加载） */
let vizGlobalPromise: Promise<void> | null = null;

/**
 * 确保 viz-global.js 已加载到全局作用域
 * 优先从 node_modules 加载，失败则回退到 unpkg CDN
 */
function ensureVizGlobal(): Promise<void> {
  // 已加载则直接返回
  if (vizGlobalLoaded && typeof window !== 'undefined' && (window as any).Viz) {
    return Promise.resolve();
  }

  // 正在加载中则复用同一 Promise
  if (vizGlobalPromise) return vizGlobalPromise;

  vizGlobalPromise = new Promise<void>((resolve, reject) => {
    // 如果已经存在全局 Viz 对象，直接完成
    if (typeof window !== 'undefined' && (window as any).Viz) {
      vizGlobalLoaded = true;
      resolve();
      return;
    }

    // 创建 script 标签加载 viz-global.js
    const script = document.createElement('script');
    // 使用 @plantuml/core 附带的 viz-global.js（Graphviz 布局引擎）
    script.src = 'https://unpkg.com/@plantuml/core/viz-global.js';
    script.async = true;
    ...
    script.onerror = () => {
      ...
      fallback.src = 'https://unpkg.com/@plantuml/core@1.2026.7/viz-global.js';
      fallback.async = true;
      ...
      fallback.onerror = () => {
        vizGlobalPromise = null; // 允许重试
        reject(new Error('无法加载 viz-global.js（Graphviz 布局引擎）。请检查网络连接。'));
      };
...
export function PlantumlRenderer({
  ...
  useEffect(() => {
    ...
    const renderPlantuml = async () => {
      ...
      try {
        // 第一步：确保 Graphviz 布局引擎已加载
        await ensureVizGlobal();

        if (cancelled) return;

        // 第二步：动态导入 @plantuml/core
        const plantumlCore = await import('@plantuml/core');
        const { renderToString } = plantumlCore;
        ...
        const svg = await new Promise<string>((resolve, reject) => {
          renderToString(
            ...
            (err: any) => {
              ...
              reject(new Error(`PlantUML 渲染失败: ${message}`));
            },
            { dark: isDark }
          );
        });
        ...
        if (!cancelled) {
          const message =
            err instanceof Error ? err.message : 'PlantUML 渲染失败';
          setError(message);
          onError?.(err instanceof Error ? err : new Error(message));
          setLoading(false);
        }
