 ...
 *
 * 功能：
 * - 动态导入 @myriaddreamin/typst.ts（SSR 兼容）
 * - 使用 $typst.svg({ mainContent }) 将 .typ 源码编译为 SVG
 * - SVG 缩放（CSS transform scale + 鼠标滚轮）
 * - 拖拽平移（pointer events，兼容触摸）
 * - 全屏切换
 * - 工具栏：缩放+/-、重置、全屏、下载 SVG、源码/预览切换
 * - 暗色/亮色主题自动适配
 * - 渲染失败时显示错误信息和原始代码
 */
...
export function TypstRenderer({
  ...
  useEffect(() => {
    ...
    const compileTypst = async () => {
      ...
      try {
        // 动态导入 typst.ts（避免 SSR 问题）
        const { $typst } = await import('@myriaddreamin/typst.ts');

        // 使用 $typst.svg 将 .typ 源码编译为 SVG
        const svg = await $typst.svg({ mainContent: rawCode });

        if (!cancelled) {
          // 对暗色主题做简单反转处理（Typst 默认白底黑字）
          let finalSvg = svg;
          if (isDark) {
            // 在 SVG 根元素上注入暗色样式：反转颜色并调整背景
        ...
        if (!cancelled) {
          const message =
            err instanceof Error ? err.message : 'Typst 编译失败';
          setError(message);
          onError?.(err instanceof Error ? err : new Error(message));
          setLoading(false);
        }
        ...
        {error && !loading && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 p-6 z-10">
            <div className="flex items-center gap-2 text-amber-500">
              ...
              <span className="text-sm font-medium">编译失败</span>
            </div>
