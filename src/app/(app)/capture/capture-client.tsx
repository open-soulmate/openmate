...
function formatTime(ts: number, t: (key: string, opts?: any) => string): string {
  const d = new Date(ts * 1000);
  const now = new Date();
  const diff = (now.getTime() - d.getTime()) / 1000;
  if (diff < 60) return t('capture.justNow');
  if (diff < 3600) return t('capture.minutesAgo', { count: Math.floor(diff / 60) });
  if (diff < 86400) return t('capture.hoursAgo', { count: Math.floor(diff / 3600) });
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      {stats && (
        <div className="grid grid-cols-3 gap-3 border-b border-border px-6 py-3">
          ...
          <div className="rounded-lg bg-blue-500/5 px-3 py-2">
            <div className="text-[10px] text-muted-foreground">{t('capture.pageCaptures')}</div>
            <div className="text-lg font-bold text-blue-500">{stats.page_captures}</div>
          </div>
          <div className="rounded-lg bg-purple-500/5 px-3 py-2">
            <div className="text-[10px] text-muted-foreground">{t('capture.textCaptures')}</div>
      ...
      <div className="flex items-center gap-2 border-b border-border px-6 py-2.5">
        <div className="flex rounded-lg border border-border p-0.5">
          {(["all", "page", "selection"] as const).map((filterItem) => (
            <button key={filterItem} onClick={() => setFilter(filterItem)}
              className={cn('rounded-md px-2.5 py-1 text-xs transition-colors', filter === filterItem ? 'bg-accent font-medium' : 'text-muted-foreground hover:text-foreground')}>
              {t === 'all' ? t2('capture.all') : t === 'page' ? t2('capture.page') : t2('capture.selection')}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-3">
        {loading && captures.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
            ...
            <p className="text-sm">{t('capture.noCaptures')}</p>
            <p className="text-xs mt-1">{t('capture.noCapturesDesc')}</p>
          </div>
        ) : (
              ...
              <div key={capture.id} className="group rounded-xl border border-border bg-card p-4 transition-colors hover:border-orange-500/30">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <h3 className="text-sm font-medium truncate">{capture.title || t('capture.noTitle')}</h3>
