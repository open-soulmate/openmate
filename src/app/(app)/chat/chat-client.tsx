  ...
  const renderDetailsContent = () => (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4">
        {selectedAgent && (
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 p-2 rounded-lg bg-muted/50">
              <span className="text-lg shrink-0">{selectedAgent.icon}</span>
              <div className="min-w-0 flex-1">
                <div className="text-sm font-medium truncate">{selectedAgent.name}</div>
                <div className="text-xs text-muted-foreground truncate" title={selectedAgent.description}>{selectedAgent.description}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Checkpoints — visible on both mobile and desktop */}
        <div>
          <div className="text-xs text-muted-foreground mb-2 flex items-center justify-between">
            ...
            <button onClick={() => setShowCheckpoints(!showCheckpoints)} className="text-primary hover:underline">
              ...
              {checkpoints.length === 0 ? (
                ...
                checkpoints.map(cp => (
                  <div key={cp.id} className="flex items-center justify-between p-2 rounded-lg bg-muted/50">
                    ...
                    <button onClick={() => rollbackToCheckpoint(cp.id)} className="ml-2 px-2 py-1 rounded text-[10px] bg-primary/10 text-primary hover:bg-primary/20 transition-colors">
                      {t("chat.rollback")}
                    </button>
                  </div>
                ))
