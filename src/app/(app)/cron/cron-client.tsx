    ...
    <div className="p-6 h-full overflow-y-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowCreate(!showCreate)} className="px-4 py-2 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 flex items-center gap-2 text-sm"><Plus className="w-4 h-4" /> {t('cron.newJob')}</button>
          <button onClick={loadJobs} className="px-4 py-2 rounded-lg border hover:bg-muted flex items-center gap-2 text-sm"><RefreshCw className="w-4 h-4" /> {t('cron.refreshBtn')}</button>
        </div>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="p-4 rounded-xl border bg-card"><p className="text-2xl font-bold text-primary">{jobs.length}</p><p className="text-sm text-muted-foreground">{t('cron.totalTasks')}</p></div>
      ...
      {showCreate && (
        <div className="mb-6 p-4 rounded-xl border bg-card">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-medium flex items-center gap-2"><Zap className="w-4 h-4 text-primary" /> {t('cron.t03413')}</h3>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t('cron.taskNameLabel')}</label>
              <input value={createName} onChange={e => setCreateName(e.target.value)} placeholder={t('cron.taskNamePlaceholder')}
          ...
          <div className="mb-4">
            <label className="text-xs text-muted-foreground mb-1 block">{t('cron.executePrompt')}</label>
          ...
          <div className="flex gap-2">
            <button onClick={handleCreate} disabled={creating || !createSchedule.trim()}
              ...
              {creating ? <Loader2 className="w-3 h-3 animate-spin" /> : <Zap className="w-3 h-3" />} {t('cron.createBtn')}
            </button>
                ...
                <button onClick={() => toggleAgent(agent)}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-muted-foreground">{t('cron.tasksCount', { count: agentJobs.length, active: agentActive })}</p>
                    </div>
                  </div>
                        ...
                        <div className="flex items-center gap-0.5 ml-3 shrink-0">
                          {job.status === 'active' ? (
                            <button onClick={() => handleAction(job.id, 'pause')} disabled={actionLoading === job.id + 'pause'}
                              className="p-1.5 rounded-lg hover:bg-muted text-amber-500" title={t('cron.pauseBtn')}>
                            ...
                            <button onClick={() => handleAction(job.id, 'resume')} disabled={actionLoading === job.id + 'resume'}
                              className="p-1.5 rounded-lg hover:bg-muted text-green-500" title={t('cron.resumeBtn')}>
                          ...
                          <button onClick={() => handleAction(job.id, 'delete')} disabled={actionLoading === job.id + 'delete'}
                            className="p-1.5 rounded-lg hover:bg-muted text-destructive" title={t('cron.deleteBtn')}>
