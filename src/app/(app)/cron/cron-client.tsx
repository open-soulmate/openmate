...
export function CronClient() {
            ...
            <div>
              <div className="flex items-center gap-1.5 px-2 py-1">
                ...
                <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">
                  {t('cron.running', '运行中')} ({groupedJobs.active.length})
                </span>
              </div>
          ...
          {groupedJobs.paused.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 px-2 py-1">
                ...
                <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wider">
                  {t('cron.paused', '已暂停')} ({groupedJobs.paused.length})
                </span>
              </div>
              <div className="space-y-0.5">
  ...
  }, [groupedJobs, selectedJobId, jobs.length, t, setPageSidebar]);
      ...
      {jobs.length === 0 ? (
        ...
        <div className="space-y-3">
          {jobs.map(job => {
            ...
            return (
              <div key={jobId} className={cn('rounded-xl border bg-card overflow-hidden cursor-pointer transition-all',
                selectedJobId === jobId ? 'border-primary ring-1 ring-primary/30' : 'border-border hover:border-primary/30'
              )} onClick={() => setSelectedJobId(jobId)}>
                {/* Job header card */}
                <div className="p-3 lg:p-4">
                  <div className="flex items-start lg:items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-0.5 ml-2 lg:ml-3 shrink-0">
                      {job.status === 'active' ? (
                        <button onClick={(e) => { e.stopPropagation(); handleAction(jobId, 'pause'); }}
                        ...
                        <button onClick={(e) => { e.stopPropagation(); handleAction(jobId, 'resume'); }}
                      ...
                      <button onClick={(e) => { e.stopPropagation(); handleAction(jobId, 'run'); }}
                      ...
                      <button onClick={(e) => { e.stopPropagation(); toggleHistory(jobId); }}
                      ...
                      <button onClick={(e) => { e.stopPropagation(); handleAction(jobId, 'delete'); }}
