          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                <span className="text-emerald-500">{data.summary.healthy}</span> {t("diagnostics.healthy"))} ·
                <span className="text-red-500 ml-1">{data.summary.unhealthy}</span> {t("diagnostics.unhealthy"))}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <div className="flex items-center justify-between mb-2">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{t("diagnostics.max"))} {data.summary.max_response_ms.toFixed(1)}ms</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <div className="flex items-center justify-between mb-2">
              ...
              <p className="text-2xl font-bold">
                {data.system.cpu_percent !== undefined ? `${data.system.cpu_percent}%` : t('diagnostics.t39409', { cpucount: data.system.cpu_count, cores: t("diagnostics.cores")) })}
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">
                {data.system.has_psutil ? t('diagnostics.t17783', { cpucount: data.system.cpu_count, cores: t("diagnostics.cores")) }) : `Load: ${data.system.load_avg_1m?.toFixed(2)}`}
              </p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <div className="flex items-center justify-between mb-2">
            ...
            <div className="px-5 py-3 border-b border-border bg-muted/30">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-muted-foreground">
                  {organs.filter((o) => o.status === "ok").length}/{organs.length} {t("diagnostics.healthy"))}
                </span>
              </div>
            </div>
                    ...
                    <div className="px-5 pb-4 pt-0">
                      <div className="rounded-lg bg-muted/50 p-4 text-xs space-y-2">
                        <div className="grid grid-cols-3 gap-2">
                          <div><span className="text-muted-foreground">{t("diagnostics.statusCode") || t('diagnostics.statusCode')}:</span> {organDetails[organ.key].status_code}</div>
                          <div><span className="text-muted-foreground">{t("diagnostics.responseTime") || t('diagnostics.responseTime')}:</span> {organDetails[organ.key].response_time_ms}ms</div>
                          <div><span className="text-muted-foreground">{t("diagnostics.category"))}:</span> {organDetails[organ.key].category}</div>
                        </div>
                        {organDetails[organ.key].error && (
                          <div className="text-red-500">{t("diagnostics.error"))}: {organDetails[organ.key].error}</div>
                        )}
            ...
            <div className="px-5 py-3 border-b border-border bg-muted/30">
              <h3 className="text-sm font-semibold flex items-center gap-2">
                ...
                <span className="rounded-full bg-violet-500/10 px-2 py-0.5 text-[10px] font-medium text-violet-500">
                  {configChanges.length} {t("diagnostics.items"))}
                </span>
              </h3>
            </div>
            <div className="divide-y divide-border">
              {configChanges.map((change) => (
                <div key={change.key} className="flex items-center gap-4 px-5 py-3 text-xs">
                  ...
                  <span className="text-muted-foreground flex-1 truncate">
                    {t("diagnostics.default"))}: <span className="font-mono">{JSON.stringify(change.default)}</span>
                  </span>
                  <span className="text-amber-500 font-mono truncate">
                    {t("diagnostics.current"))}: {JSON.stringify(change.current)}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className={cn(
            ...
            <div>
              <p className="text-sm font-semibold">
                {data.summary.overall === "ok" ? (t("diagnostics.allOk"))) : (t("diagnostics.partialError") || t('diagnostics.partialError'))}
              </p>
              <p className="text-xs text-muted-foreground mt-0.5">
                {data.summary.healthy}/{data.summary.total} {t("diagnostics.organHealth") || t('diagnostics.organHealth')} · {t("diagnostics.avgResponse")} {data.summary.avg_response_ms}ms ·
                {t("diagnostics.system"))} {data.system.hostname} ({data.system.os})
              </p>
            </div>
          </div>
        )}
