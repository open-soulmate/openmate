...
export function EchoClient() {
  ...
  const tabs = [
    { id: "send" as const, label: "发送消息", icon: Send },
    { id: "channels" as const, label: "通道管理", icon: Radio },
        ...
        {health && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">总消息</span>
        ...
        {tab === "send" && (
          <div className="space-y-4 max-w-xl">
            ...
            <input value={sendTitle} onChange={(e) => setSendTitle(e.target.value)}
              placeholder="消息标题" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
            <textarea value={sendContent} onChange={(e) => setSendContent(e.target.value)}
              placeholder="消息内容" className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm min-h-[100px] resize-none" />
            <div className="flex gap-2">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">通道</th>
