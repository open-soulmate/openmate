    ...
    <div className="flex w-56 shrink-0 flex-col border-r border-border bg-sidebar">
      <div className="border-b border-border px-3 py-3">
        ...
        <p className="mt-1 text-[11px] text-muted-foreground">
          {members.length} {t("aiGroups.ab5dea")} · {members.filter((m) => {t("aiGroups.e2fac4")}
        <p>
      </div>
      ...
      <div className="border-t border-border px-3 py-2">
        <p className="text-[10px] text-muted-foreground">
          {t("aiGroups.0161f1")} {group.dispatchMode === "auto" ? t("aiGroups.fc0af0") : t("aiGroups.dc80d2")}
        </p>
      </div>
    </div>
  );
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <span className="text-[10px] text-muted-foreground">
          {agent.status === "online" ? t("aiGroups.68905c") : agent.status === "error" ? t("aiGroups.7030ff") : t("aiGroups.50d4a8")}
        </span>
      </div>
    </div>
  );
  ...
  if (!group) {
    return (
      <div className="flex h-full flex-col items-center justify-center">
        ...
        <h3 className="mb-2 text-sm font-medium">{t("aiGroups.c7a773")}<h3>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">
                {group.memberAgentIds.length} {t("aiGroups.ab5dea")} · {group.dispatchMode === "auto" ? t("aiGroups.fc0af0") : t("aiGroups.dc80d2")}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              <Play size={12} />
              {meetingActive ? t("aiGroups.73f81a") : t("aiGroups.43f7ed")}
            </button>
            <button
              onClick={() => clearGroupMessages(groupId)}
              className="flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:bg-accent hover:text-foreground"
              title={t("aiGroups.cf60d5")}
            >
          ...
          {messages.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center">
              ...
              <p className="max-w-sm text-sm text-muted-foreground mb-4">
                {group.description || t("aiGroups.af8e66")}
              </p>
              <div className="flex flex-wrap justify-center gap-2">
                <button
                  onClick={() => {t("aiGroups.e01b30")}
                <button>
                <button
                  onClick={startMeeting}
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[11px] text-muted-foreground">
                        {agent.type} · {agent.status === "online" ? t("aiGroups.68905c") : t("aiGroups.50d4a8")}
                      </span>
                    </button>
                  );
                })}
            ...
            <div className="mt-2 flex items-center justify-between">
              <p className="text-[11px] text-muted-foreground">
                {meetingActive ? (
                  ...
                  <span>
                    {t("aiGroups.494ce5")} {master?.name ?? t("aiGroups.fe2d26")} · {t("aiGroups.ecff77")} @Agent{t("aiGroups.1f1ba4")}
                  </span>
                )}
              </p>
              {selectedMention && (
