      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {platform && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard icon={Database} label={t("heredity.totalComponents")} value={String(platform.total_components)}
              sub={t("heredity.registered")} color="text-teal-500" bg="bg-teal-500/10" />
            <StatCard icon={ArrowUpCircle} label={t("heredity.totalMigrations")} value={String(platform.total_migrations)}
              sub={t("heredity.plans")} color="text-blue-500" bg="bg-blue-500/10" />
            <StatCard icon={History} label={t("heredity.changelogEntries")} value={String(platform.total_changelog_entries)}
              sub={t("heredity.entries")} color="text-violet-500" bg="bg-violet-500/10" />
            <StatCard icon={Tag} label={t("heredity.platformVersion")} value={`v${platform.platform_version}`}
              sub={t("heredity.currentRelease")} color="text-emerald-500" bg="bg-emerald-500/10" />
          </div>
        )}
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1 max-w-xs">
                ...
                <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder={t("heredity.searchComponents")}
              ...
              <span className="text-xs text-muted-foreground">{filteredComponents.length} {t("heredity.componentsCount")}</span>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              {componentDetail && (
                <div className="rounded-xl border border-border bg-card p-5 space-y-4">
                  ...
                  {/* Version History */}
                  <div>
                    <h4 className="text-xs text-muted-foreground mb-2">{t("heredity.versionHistory")}</h4>
        ...
        {tab === "migrations" && (
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-border p-4 space-y-3">
              <h3 className="text-sm font-medium">{t("heredity.createMigration")}</h3>
              <div className="grid grid-cols-4 gap-3">
                <select value={migComponentId} onChange={(e) => setMigComponentId(e.target.value)}
                  className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none">
                  <option value="">{t("heredity.selectComponent")}</option>
                  {components.map((c) => (
        ...
        {tab === "platform" && platform && (
          <div className="space-y-4">
            <div className="rounded-xl border border-border bg-card p-6 space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold">{t("heredity.platformVersion")}</h3>
            ...
            <div className="rounded-xl border border-border bg-card p-5 space-y-3">
              <h3 className="text-sm font-semibold">{t("heredity.dependencyGraph")}</h3>
              <div className="space-y-2">
                {components.map((c) => (
