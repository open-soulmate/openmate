...
export function HeredityClient() {
  ...
  const tabs = [
    { id: "components" as const, label: t("heredity.components") || t("heredity.version"), icon: Database },
    { id: "migrations" as const, label: t("heredity.migrations") || t("heredity.text1"), icon: ArrowUpCircle },
    { id: "changelog" as const, label: t("heredity.changelog") || t("heredity.text2"), icon: History },
    { id: "platform" as const, label: t("heredity.platform") || t("heredity.text3"), icon: Settings },
  ];
        ...
        {platform && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard icon={Database} label={t("heredity.totalComponents") || t("heredity.text6")} value={String(platform.total_components)}
              sub={t("heredity.registered") || t("heredity.register")} color="text-teal-500" bg="bg-teal-500/10" />
            <StatCard icon={ArrowUpCircle} label={t("heredity.totalMigrations") || t("heredity.text7")} value={String(platform.total_migrations)}
              sub={t("heredity.plans") || t("heredity.text8")} color="text-blue-500" bg="bg-blue-500/10" />
            <StatCard icon={History} label={t("heredity.changelogEntries") || t("heredity.text9")} value={String(platform.total_changelog_entries)}
              sub={t("heredity.entries") || t("heredity.text10")} color="text-violet-500" bg="bg-violet-500/10" />
            <StatCard icon={Tag} label={t("heredity.platformVersion") || t("heredity.text3")} value={`v${platform.platform_version}`}
              sub={t("heredity.currentRelease") || t("heredity.current")} color="text-emerald-500" bg="bg-emerald-500/10" />
          </div>
        )}
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1 max-w-xs">
                ...
                <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder={t("heredity.searchComponents") || t("heredity.search")}
              ...
              <span className="text-xs text-muted-foreground">{filteredComponents.length} {t("heredity.componentsCount") || t("heredity.text11")}</span>
            </div>
                ...
                {filteredComponents.map((c) => (
                  <button key={c.component_id}
                    ...
                    <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
                      ...
                      <span>{c.total_versions} {t("heredity.versions") || t("heredity.text12")}</span>
                      <span>{Object.keys(c.dependencies).length} {t("heredity.deps") || t("heredity.text13")}</span>
                    </div>
                  </button>
                ))}
              </div>
              ...
              {componentDetail && (
                <div className="rounded-xl border border-border bg-card p-5 space-y-4">
                  ...
                  {compatCheck && (
                    <div className={cn("rounded-lg border p-3 text-sm",
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">{compatCheck.compatible ? t("heredity.text14") : t("heredity.text15")}</span>
                      </div>
                      {compatCheck.issues?.map((issue: any, i: number) => (
                        <p key={i} className="mt-1 text-xs text-red-400">{issue.dependency}: {issue.issue} ({t("heredity.text16")} {issue.required}, {t("heredity.text17")} {issue.actual})</p>
                      ))}
                    </div>
                  )}
                  ...
                  {Object.keys(componentDetail.current.dependencies).length > 0 && (
                    <div>
                      <h4 className="text-xs text-muted-foreground mb-2">{t("heredity.dependencies") || t("heredity.text13")}</h4>
                  ...
                  {/* Version History */}
                  <div>
                    <h4 className="text-xs text-muted-foreground mb-2">{t("heredity.versionHistory") || t("heredity.text18")}</h4>
                  ...
                  {componentDetail.current.release_notes && (
                    <div>
                      <h4 className="text-xs text-muted-foreground mb-2">{t("heredity.releaseNotes") || t("heredity.text19")}</h4>
        ...
        {tab === "migrations" && (
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-border p-4 space-y-3">
              <h3 className="text-sm font-medium">{t("heredity.createMigration") || t("heredity.create")}</h3>
              <div className="grid grid-cols-4 gap-3">
                <select value={migComponentId} onChange={(e) => setMigComponentId(e.target.value)}
                  className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none">
                  <option value="">{t("heredity.selectComponent") || t("heredity.select")}</option>
                  {components.map((c) => (
                ...
                <input value={migFrom} onChange={(e) => setMigFrom(e.target.value)}
                  placeholder={t("heredity.fromVersion") || t("heredity.text20")}
                ...
                <input value={migTo} onChange={(e) => setMigTo(e.target.value)}
                  placeholder={t("heredity.toVersion") || t("heredity.text21")}
                  className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-teal-500/20" />
                ...
                {migrations.map((m) => (
                  <div key={m.migration_id} className="rounded-xl border border-border bg-card p-4">
                      ...
                      <div className="flex items-center gap-2">
                        {(m.status === "pending") && (
                          <button onClick={() => handleExecuteMigration(m.migration_id)} disabled={loading}
                            ...
                            <Play size={12} /> {t("heredity.execute") || t("heredity.text23")}
                          </button>
                        )}
                        {(m.status === "completed" || m.status === "failed") && (
                          <button onClick={() => handleRollbackMigration(m.migration_id)} disabled={loading}
                            ...
                            <RotateCcw size={12} /> {t("heredity.rollback") || t("heredity.text24")}
                          </button>
                        )}
                      </div>
                    </div>
                    <div className="mt-2 flex items-center gap-4 text-xs text-muted-foreground">
                      ...
                      {m.error && <span className="text-red-400">{t("heredity.error") || t("heredity.text25")}: {m.error}</span>}
                    </div>
                  </div>
                ))}
              </div>
        ...
        {tab === "platform" && platform && (
          <div className="space-y-4">
            <div className="rounded-xl border border-border bg-card p-6 space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold">{t("heredity.platformVersion") || t("heredity.text3")}</h3>
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                <div className="rounded-lg bg-muted p-4">
                  ...
                  <p className="text-xs text-muted-foreground">{t("heredity.components") || t("heredity.text27")}</p>
                </div>
                <div className="rounded-lg bg-muted p-4">
                  <p className="text-2xl font-bold">{platform.total_migrations}</p>
                  <p className="text-xs text-muted-foreground">{t("heredity.migrations") || t("heredity.text28")}</p>
                </div>
                <div className="rounded-lg bg-muted p-4">
                  <p className="text-2xl font-bold">{platform.total_changelog_entries}</p>
                  <p className="text-xs text-muted-foreground">{t("heredity.changes") || t("heredity.text29")}</p>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-card p-5 space-y-3">
              <h3 className="text-sm font-semibold">{t("heredity.dependencyGraph") || t("heredity.text30")}</h3>
              <div className="space-y-2">
                {components.map((c) => (
                  <div key={c.component_id} className="flex items-center gap-3 text-sm">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {Object.keys(c.dependencies).length === 0 ? (
                        <span className="text-xs text-muted-foreground">{t("heredity.noDeps") || t("heredity.text31")}</span>
                      ) : (
