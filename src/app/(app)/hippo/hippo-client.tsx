...
function formatAge(ts: number): string {
  ...
  if (hours < 1) return t('hippo.t12041', { roundhours60: Math.round(hours * 60) });
  if (hours < 24) return t('hippo.t09995', { roundhours: Math.round(hours) });
  return t('hippo.t95469', { roundhours24: Math.round(hours / 24) });
}
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard icon={Brain} label={t("hippo.activeMemories"))} value={String(stats.memory.active)}
              sub={t('hippo.t31777', { totalmemories: stats.memory.total_memories, total: t("hippo.total")) })} color="text-indigo-500" bg="bg-indigo-500/10" />
            <StatCard icon={Archive} label={t("hippo.archived"))} value={String(stats.memory.archived)}
              sub={t('hippo.t23942', { usagepercent: stats.memory.usage_percent, usage: t("hippo.usage")) })} color="text-amber-500" bg="bg-amber-500/10" />
            <StatCard icon={Clock} label={t("hippo.sessions"))} value={String(stats.sessions.total_sessions)}
              sub={t('hippo.t85391', { active0: stats.sessions.by_status?.active || 0, active: t("hippo.active")) })} color="text-emerald-500" bg="bg-emerald-500/10" />
            <StatCard icon={Activity} label={t("hippo.totalAccesses"))} value={String(stats.memory.total_accesses)}
              sub={t('hippo.t37700', { strategy: t("hippo.strategy")), strategy1: stats.memory.strategy })} color="text-violet-500" bg="bg-violet-500/10" />
          </div>
        )}
            ...
            <div className="rounded-xl border border-border p-4 space-y-3">
              ...
              <div className="flex gap-3">
                ...
                <input value={newContent} onChange={(e) => setNewContent(e.target.value)}
                  placeholder={t("hippo.contentPlaceholder") || t('hippo.t90709')}
            ...
            {decayConfig && (
              <div className="rounded-xl border border-border bg-card p-5 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-xs text-muted-foreground">{t("hippo.archiveThreshold") || t('hippo.archiveThreshold')}</label>
                ...
                <div className="mt-4 rounded-lg border border-border p-4 space-y-2">
                  ...
                  <div className="flex gap-4 text-xs">
                    <span className={simResult.should_archive ? "text-amber-500" : "text-muted-foreground"}>
                      {simResult.should_archive ? t('hippo.t38102') : t('hippo.t90923')}
                    </span>
                    <span className={simResult.should_forget ? "text-red-500" : "text-muted-foreground"}>
                      {simResult.should_forget ? t('hippo.t17561') : t('hippo.t35088')}
                    </span>
                  </div>
                </div>
              )}
