...
function formatAge(ts: number): string {
  ...
  if (hours < 1) return `${Math.round(hours * 60)}{t("hippo.text1")}`;
  if (hours < 24) return `${Math.round(hours)}{t("hippo.text2")}`;
  return `${Math.round(hours / 24)}{t("hippo.text3")}`;
}
...
export function HippoClient() {
  ...
  const handleRunDecay = async () => {
    ...
    try {
      ...
      const data = await res.json();
      alert(`{t("hippo.done")}: {t("hippo.update")}${data.updated}{t("hippo.text4")}, {t("hippo.text5")}${data.archived}条, {t("hippo.text6")}${data.forgotten}条`);
      fetchMemories();
      fetchStats();
    } catch {} finally { setLoading(false); }
  };
  ...
  const handleLifecycleCheck = async () => {
    try {
      ...
      const data = await res.json();
      alert(`{t("hippo.text7")}: {t("hippo.text8")}${data.checked}{t("hippo.text9")}, {t("hippo.text10")}${data.newly_idle}{t("hippo.text11")}, {t("hippo.text12")}${data.newly_expired}个`);
      fetchSessions();
    } catch {}
  };

  const tabs = [
    { id: "memories" as const, label: t("hippo.memories") || t("hippo.text13"), icon: Brain },
    { id: "sessions" as const, label: t("hippo.sessions") || t("hippo.text14"), icon: Clock },
    { id: "decay" as const, label: t("hippo.decay") || t("hippo.text15"), icon: TrendingDown },
    { id: "simulate" as const, label: t("hippo.simulate") || t("hippo.text16"), icon: BarChart3 },
  ];
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-border px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold">{t("hippo.title") || t("hippo.text17")}</h1>
          <span className="rounded-full bg-indigo-500/10 px-2 py-0.5 text-xs font-medium text-indigo-500">
            {t("hippo.subtitle") || t("hippo.text18")}
          </span>
        </div>
        <button onClick={() => { fetchStats(); tab === "memories" && fetchMemories(); tab === "sessions" && fetchSessions(); }}
          ...
          <RefreshCw size={14} /> {t("common.refresh") || t("hippo.refresh")}
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard icon={Brain} label={t("hippo.activeMemories") || t("hippo.text19")} value={String(stats.memory.active)}
              sub={`${stats.memory.total_memories} ${t("hippo.total") || t("hippo.text20")}`} color="text-indigo-500" bg="bg-indigo-500/10" />
            <StatCard icon={Archive} label={t("hippo.archived") || t("hippo.text21")} value={String(stats.memory.archived)}
              sub={`${stats.memory.usage_percent}% ${t("hippo.usage") || t("hippo.text22")}`} color="text-amber-500" bg="bg-amber-500/10" />
            <StatCard icon={Clock} label={t("hippo.sessions") || t("hippo.text14")} value={String(stats.sessions.total_sessions)}
              sub={`${stats.sessions.by_status?.active || 0} ${t("hippo.active") || t("hippo.text23")}`} color="text-emerald-500" bg="bg-emerald-500/10" />
            <StatCard icon={Activity} label={t("hippo.totalAccesses") || t("hippo.text24")} value={String(stats.memory.total_accesses)}
              sub={`${t("hippo.strategy") || t("hippo.text25")}: ${stats.memory.strategy}`} color="text-violet-500" bg="bg-violet-500/10" />
          </div>
        )}
        ...
        {tab === "memories" && (
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-border p-4 space-y-3">
              <h3 className="text-sm font-medium">{t("hippo.createMemory") || t("hippo.create")}</h3>
              <div className="flex gap-3">
                <input value={newSessionId} onChange={(e) => setNewSessionId(e.target.value)}
                  placeholder={t("hippo.sessionId") || t("hippo.text26")}
                ...
                <input value={newContent} onChange={(e) => setNewContent(e.target.value)}
                  placeholder={t("hippo.contentPlaceholder") || t("hippo.content")}
                ...
                <div className="flex items-center gap-2">
                  <label className="text-xs text-muted-foreground">{t("hippo.importance") || t("hippo.text27")}</label>
            ...
            <div className="flex items-center gap-3">
              <div className="relative flex-1 max-w-sm">
                ...
                <input value={searchSession} onChange={(e) => setSearchSession(e.target.value)}
                  ...
                  placeholder={t("hippo.filterSession") || t("hippo.text28")}
              ...
              <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer">
                ...
                {t("hippo.includeArchived") || t("hippo.text29")}
              </label>
              <span className="text-xs text-muted-foreground">{memories.length} {t("hippo.entries") || t("hippo.text4")}</span>
            </div>
            ...
            {memories.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                ...
                <p className="text-sm">{t("hippo.noMemories") || t("hippo.empty")}</p>
              </div>
            ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                          ...
                          <span>{t("hippo.accessed") || t("hippo.text30")}: {m.access_count}</span>
                          {m.archived && <span className="text-amber-500 font-medium">{t("hippo.text21")}</span>}
                        </div>
                      </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <button onClick={handleLifecycleCheck}
                ...
                <Zap size={14} /> {t("hippo.lifecycleCheck") || t("hippo.text7")}
              </button>
            ...
            {sessionList.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                ...
                <p className="text-sm">{t("hippo.noSessions") || t("hippo.text31")}</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      ...
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("hippo.title") || t("hippo.text32")}</th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("hippo.status") || t("hippo.text33")}</th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("hippo.memories") || t("hippo.text13")}</th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("hippo.lastActive") || t("hippo.text34")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {sessionList.map((s) => (
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <button onClick={handleRunDecay} disabled={loading}
                ...
                {t("hippo.runDecay") || t("hippo.text35")}
              </button>
            </div>

            {decayConfig && (
              <div className="rounded-xl border border-border bg-card p-5 space-y-4">
                <h3 className="text-sm font-semibold">{t("hippo.decayConfig") || t("hippo.config")}</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs text-muted-foreground">{t("hippo.strategy") || t("hippo.text25")}</label>
                    <div className="mt-1 text-sm font-medium">{decayConfig.strategy}</div>
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground">{t("hippo.halfLife") || t("hippo.text36")}</label>
                    <div className="mt-1 text-sm font-medium">{decayConfig.half_life_hours}h</div>
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground">{t("hippo.archiveThreshold") || t("hippo.text37")}</label>
                    <div className="mt-1 text-sm font-medium">{Math.round(decayConfig.archive_threshold * 100)}%</div>
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground">{t("hippo.forgetThreshold") || t("hippo.text38")}</label>
                ...
                <div className="mt-4">
                  <h4 className="text-xs text-muted-foreground mb-2">{t("hippo.decayCurve") || t("hippo.text39")}</h4>
        ...
        {tab === "simulate" && (
          <div className="space-y-4">
            <div className="rounded-xl border border-border bg-card p-5 space-y-4">
              <h3 className="text-sm font-semibold">{t("hippo.simulateDecay") || t("hippo.text40")}</h3>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="text-xs text-muted-foreground">{t("hippo.ageHours") || t("hippo.text41")}</label>
                ...
                <div>
                  <label className="text-xs text-muted-foreground">{t("hippo.accessCount") || t("hippo.text42")}</label>
              ...
              <button onClick={handleSimulate}
                ...
                {t("hippo.runSimulation") || t("hippo.run")}
              </button>
              ...
              {simResult && (
                <div className="mt-4 rounded-lg border border-border p-4 space-y-2">
                  <div className="flex items-center gap-4">
                    <div>
                      <span className="text-xs text-muted-foreground">{t("hippo.retention") || t("hippo.text43")}: </span>
                  ...
                  <div className="flex gap-4 text-xs">
                    <span className={simResult.should_archive ? "text-amber-500" : "text-muted-foreground"}>
                      {simResult.should_archive ? t("hippo.text44") : t("hippo.text45")}
                    </span>
                    <span className={simResult.should_forget ? "text-red-500" : "text-muted-foreground"}>
                      {simResult.should_forget ? t("hippo.text46") : t("hippo.text47")}
                    </span>
                  </div>
                </div>
              )}
