...
export function SharingTab() {
  ...
  return (
    <div className="p-3 lg:p-6 space-y-4 lg:space-y-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-muted-foreground text-xs">{t('kbSharing.description', 'Share knowledge bases with team members')}</p>
        </div>
        <button onClick={() => setShowCreate(!showCreate)} className="px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 flex items-center gap-2 text-xs">
      ...
      {showCreate && (
        <div className="p-4 border rounded-lg bg-card space-y-4">
          <h3 className="font-medium text-sm">{t('kbSharing.createTitle', 'Create Sharing Request')}</h3>
          ...
          <div className="flex gap-2">
            <button onClick={handleCreate} disabled={submitting} className="px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 disabled:opacity-50 flex items-center gap-2 text-xs">
              {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />} {t('common.submit', 'Submit')}
            </button>
            <button onClick={() => setShowCreate(false)} className="px-4 py-2 border rounded-md hover:bg-accent text-xs">{t('common.cancel', 'Cancel')}</button>
          </div>
        </div>
      )}
        ...
        <div className="flex gap-2">
          {['', 'pending', 'approved', 'rejected'].map((s) => (
            <button key={s} onClick={() => setFilter(s)} className={cn(
              'px-3 py-1.5 rounded-md text-xs lg:text-sm transition-colors',
              filter === s ? 'bg-primary text-primary-foreground' : 'bg-secondary hover:bg-secondary/80 text-muted-foreground'
            )}>
              {s || t('common.all', 'All')}
            </button>
          ))}
              ...
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-sm">{req.kb_name}</span>
