...
export function LimbClient() {
  ...
  const tabs = [
    { id: "tasks" as const, label: t("limb.tasks") || "任务", icon: Terminal },
    { id: "templates" as const, label: t("limb.templates") || "模板", icon: FileCode },
    { id: "history" as const, label: t("limb.history") || "历史", icon: Clock },
  ];
    ...
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex items-center justify-between border-b border-border px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold">{t("limb.title") || "四肢 · RPA执行器"}</h1>
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("limb.t26095") || "总任务"}</span>
              <p className="text-2xl font-bold">{stats.total_tasks}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("limb.running") || "运行中"}</span>
              <p className="text-2xl font-bold text-blue-500">{stats.running}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("limb.t54694") || "队列"}</span>
              <p className="text-2xl font-bold text-amber-500">{stats.queue_length}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("limb.successRate") || "成功率"}</span>
              <p className="text-2xl font-bold text-emerald-500">{stats.success_rate}%</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("limb.templates") || "模板"}</span>
            ...
            <div className="flex-1 space-y-3">
              {tasks.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                  ...
                  <p className="text-sm">{t("limb.t14143") || "暂无任务，从模板创建一个吧"}</p>
                </div>
              ) : tasks.map((task) => (
              ...
              <div className="w-96 space-y-4">
                <div className="rounded-xl border border-border bg-card p-4 space-y-3">
                  ...
                  <div className="space-y-1 text-xs text-muted-foreground">
                    ...
                    <div className="flex justify-between">
                      <span>{t("limb.status") || "状态"}</span><span className={statusColor(selected.status)}>{selected.status}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{t("limb.t97513") || "进度"}</span><span>{selected.progress}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{t("limb.elapsed") || "耗时"}</span><span>{selected.elapsed_seconds}s</span>
                    </div>
                  </div>
                  {selected.error && (
                ...
                {selected.results && selected.results.length > 0 && (
                  <div className="rounded-xl border border-border bg-card p-4 space-y-2">
                    <h4 className="text-sm font-medium">{t("limb.t62005") || "执行步骤"}</h4>
                ...
                {selected.actions && selected.actions.length > 0 && (
                  <div className="rounded-xl border border-border bg-card p-4 space-y-2">
                    <h4 className="text-sm font-medium">{t("limb.t47867") || "动作列表"}</h4>
            ...
            {templates.map((tpl) => (
              <div key={tpl.template_id}
                ...
                <div className="flex gap-2 text-xs text-muted-foreground">
                  <span>{(t("limb.actionCount") || "{count} 个动作").replace("{count}", String(tpl.action_count))}</span>
                  <span>{(t("limb.variableCount") || "{count} 个变量").replace("{count}", String(tpl.variables.length))}</span>
                </div>
                ...
                <button onClick={() => setShowTemplate(tpl)}
                  ...
                  <Zap size={14} /> {t("limb.t26840") || "使用模板"}
                </button>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-3">
            {history.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                ...
                <p className="text-sm">{t("limb.noHistory") || "暂无执行历史"}</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("limb.time") || "时间"}</th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("limb.name") || "名称"}</th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("limb.status") || "状态"}</th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("limb.steps") || "步骤"}</th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("limb.elapsed") || "耗时"}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {history.map((h, i) => (
        ...
        {showTemplate && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 space-y-4">
              <h3 className="font-semibold">{t("limb.t26840") || "使用模板"}: {showTemplate.name}</h3>
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => { setShowTemplate(null); setTplVars({}); }}
                  className="rounded-lg border border-border px-4 py-2 text-sm hover:bg-muted">{t("limb.cancel") || "取消"}</button>
                <button onClick={() => handleInstantiate(showTemplate.template_id)} disabled={loading}
                  ...
                  {loading ? <Loader2 size={14} className="animate-spin" /> : (t("limb.createTask") || "创建任务")}
                </button>
              </div>
            </div>
          </div>
