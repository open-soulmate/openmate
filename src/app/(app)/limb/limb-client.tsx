...
export function LimbClient() {
  ...
  const tabs = [
    { id: "tasks" as const, label: t("limb.0e46d8"), icon: Terminal },
    { id: "templates" as const, label: t("limb.59cf15"), icon: FileCode },
    { id: "history" as const, label: t("limb.c827d8"), icon: Clock },
  ];
    ...
    <div className="flex h-full flex-col overflow-hidden">
      <div className="flex items-center justify-between border-b border-border px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold">{t("limb.d49a0f")}<h1>
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("limb.0f7991")}<span>
              <p className="text-2xl font-bold">{stats.total_tasks}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("limb.d679ae")}<span>
              <p className="text-2xl font-bold text-blue-500">{stats.running}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("limb.0796f7")}<span>
              <p className="text-2xl font-bold text-amber-500">{stats.queue_length}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("limb.b66435")}<span>
              <p className="text-2xl font-bold text-emerald-500">{stats.success_rate}%</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("limb.59cf15")}<span>
            ...
            <div className="flex-1 space-y-3">
              {tasks.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                  ...
                  <p className="text-sm">{t("limb.9e4e50")}<p>
                </div>
              ) : tasks.map((task) => (
              ...
              <div className="w-96 space-y-4">
                <div className="rounded-xl border border-border bg-card p-4 space-y-3">
                  ...
                  <div className="space-y-1 text-xs text-muted-foreground">
                    ...
                    <div className="flex justify-between">
                      <span>{t("limb.3fea7c")}<span><span className={statusColor(selected.status)}>{selected.status}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{t("limb.c7bff7")}<span><span>{selected.progress}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>{t("limb.39f137")}<span><span>{selected.elapsed_seconds}s</span>
                    </div>
                  </div>
                  {selected.error && (
                ...
                {selected.results && selected.results.length > 0 && (
                  <div className="rounded-xl border border-border bg-card p-4 space-y-2">
                    <h4 className="text-sm font-medium">{t("limb.624141")}<h4>
                ...
                {selected.actions && selected.actions.length > 0 && (
                  <div className="rounded-xl border border-border bg-card p-4 space-y-2">
                    <h4 className="text-sm font-medium">{t("limb.5229e6")}<h4>
            ...
            {templates.map((tpl) => (
              <div key={tpl.template_id}
                ...
                <div className="flex gap-2 text-xs text-muted-foreground">
                  <span>{tpl.action_count} {t("limb.8cefbe")}</span>
                  <span>{tpl.variables.length} {t("limb.6f07d8")}</span>
                </div>
          ...
          <div className="space-y-3">
            {history.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                ...
                <p className="text-sm">{t("limb.5ea62b")}<p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/30">
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("limb.19fcb9")}<th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("limb.d7ec2d")}<th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("limb.3fea7c")}<th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("limb.52b365")}<th>
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">{t("limb.39f137")}<th>
                    </tr>
                  </thead>
                  <tbody>
                    {history.map((h, i) => (
        ...
        {showTemplate && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 space-y-4">
              <h3 className="font-semibold">{t("limb.61b707")}: {showTemplate.name}</h3>
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => { setShowTemplate(null); setTplVars({}); }}
                  className="rounded-lg border border-border px-4 py-2 text-sm hover:bg-muted">{t("limb.625fb2")}<button>
                <button onClick={() => handleInstantiate(showTemplate.template_id)} disabled={loading}
                  ...
                  {loading ? <Loader2 size={14} className="animate-spin" /> {t("limb.33e8eb")}
                <button>
              </div>
            </div>
          </div>
