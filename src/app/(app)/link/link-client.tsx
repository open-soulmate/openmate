...
export function LinkClient() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm(t('link.t56760'))) return;
    try {
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {showEvents && (
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            {events.length === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-4">{t('nerve.noEvents')}<p>
            ) : (
            ...
            {connectors.length === 0 ? (
              ...
              return (
                <div key={c.connector_id}
                  ...
                  <div className="text-xs text-muted-foreground mt-1">{t('link.t44128')}: {c.event_count}</div>
                </div>
              );
            })}
          </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleTest(selected.connector_id)}
                    ...
                    <Activity size={12} />  {t('link.t33201')}
                  <button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 space-y-4">
              ...
              <select value={newType} onChange={(e) => setNewType(e.target.value)}
                ...
                <option value="oa_system">OA {t('link.t57289')}</option>
