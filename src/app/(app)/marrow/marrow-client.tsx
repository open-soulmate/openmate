...
export function MarrowClient() {
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showMsg("ok", t('marrow.t11928', { backupCreated: t("marrow.backupCreated")), backupid: data.backup_id, filecount: data.file_count, files: t("marrow.files")) }))
  ...
  const handleRestore = async (backupId: string) => {
    if (!confirm(t('marrow.t17863', { confirmRestoreBackup: t("marrow.confirmRestoreBackup") || t('marrow.confirmRestoreBackup'), backupId: backupId, willOverwrite: t("marrow.willOverwrite")) }))) return
  ...
  const handleDeleteBackup = async (backupId: string) => {
    if (!confirm(t('marrow.t79828', { confirmDeleteBackup: t("marrow.confirmDeleteBackup") || t('marrow.confirmDeleteBackup'), backupId: backupId }))) return
  ...
  const handleCreateSchedule = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        showMsg("ok", t('marrow.t23206', { scheduleCreated: t("marrow.scheduleCreated") || t('marrow.scheduleCreated'), name: data.name }))
  ...
  const handleDeleteSchedule = async (scheduleId: string) => {
    if (!confirm(t("marrow.confirmDeleteSchedule") || t('marrow.confirmDeleteSchedule'))) return
    try {
      await fetch(`${apiBase}/api/marrow/schedules/${scheduleId}`, { method: "DELETE" })
      showMsg("ok", t("marrow.scheduleDeleted") || t('marrow.scheduleDeleted'))
      refresh()
    } catch (e: any) {
      showMsg("err", e.message || (t("common.deleteFailed"))))
    }
  ...
  const handleRunDue = async () => {
    try {
      ...
      const data = await res.json()
      showMsg("ok", t('marrow.t50154', { runComplete: t("marrow.runComplete")), count: data.count, tasks: t("marrow.tasks")) }))
      refresh()
    } catch (e: any) {
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        showMsg("ok", t('marrow.t25285', { importSuccess: t("marrow.importSuccess")), records: data.records, records1: t("marrow.records")) }))
      } else {
        const err = await res.json()
  ...
  const formatInterval = (expr: string, secs: number) => {
    ...
    return map[expr] || t('marrow.t54582', { every: t("marrow.every")), secs: secs, seconds: t("marrow.seconds")) })
  }
  ...
  const formatNextRun = (ts: number) => {
    ...
    if (diff < 3600000) return t('marrow.t69373', { rounddiff60000: Math.round(diff / 60000), minutesLater: t("marrow.minutesLater")) })
    if (diff < 86400000) return t('marrow.t29381', { rounddiff3600000: Math.round(diff / 3600000), hoursLater: t("marrow.hoursLater")) })
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-6">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              >
                ...
                <div className="text-xs text-muted-foreground">{t("marrow.snapshotCurrent") || t('marrow.snapshotCurrent')}</div>
              </button>
              <button
              ...
              >
                ...
                <div className="text-sm font-semibold text-foreground">{t("marrow.scheduledBackup") || t('marrow.scheduledBackup')}</div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="col-span-2">
                    ...
                    <input value={backupForm.description} onChange={e => setBackupForm({ ...backupForm, description: e.target.value })}
                      placeholder={t("marrow.descriptionPlaceholder") || t('marrow.descriptionPlaceholder')} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
                  </div>
                </div>
        ...
        {activeTab === "schedules" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-foreground">{t("marrow.scheduledBackups") || t('marrow.scheduledBackup')}</h3>
              <div className="flex gap-2">
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    if (res.ok) {
                      const data = await res.json()
                      showMsg("ok", t('marrow.t23389', { exportSuccess: t("marrow.exportSuccess")), records: data.records, records1: t("marrow.records")) }))
                      refresh()
                    }
