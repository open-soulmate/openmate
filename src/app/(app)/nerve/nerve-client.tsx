...
export function NerveClient() {
  ...
  const tabs: Array<{ key: ActiveTab; label: string; icon: typeof Zap }> = [
    ...
    { key: "events", label: t("nerve.text2"), icon: MessageSquare },
    { key: "subscriptions", label: t("nerve.text3"), icon: Radio },
    { key: "nodes", label: t("nerve.text4"), icon: Server },
    { key: "publish", label: t("nerve.text5"), icon: Send },
  ]
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold">⚡ Nerve — {t("nerve.text6")}</h1>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: t("nerve.text7"), value: bus?.total_events ?? 0, icon: MessageSquare, color: "blue" },
          { label: t("nerve.text8"), value: bus?.total_nodes ?? 0, icon: Server, color: "purple" },
          { label: t("nerve.online"), value: bus?.online_nodes ?? 0, icon: Wifi, color: "green" },
          { label: t("nerve.text9"), value: bus?.total_subscriptions ?? 0, icon: Radio, color: "yellow" },
        ].map(s => {
          const Icon = s.icon
          return (
          ...
          {bus?.topics && bus.topics.length > 0 && (
            <div className="bg-card border border-border rounded-xl p-5">
              <h3 className="font-semibold mb-3 flex items-center gap-2">
                <Globe className="w-4 h-4" /> {t("nerve.active")}
              </h3>
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold flex items-center gap-2">
                <Clock className="w-4 h-4" /> {t("nerve.text13")}
              </h3>
              <button
      ...
      {activeTab === "events" && (
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-muted-foreground"> {t("nerve.text15")} {events.length} {t("nerve.text16")}</span>
          </div>
                          ...
                          <div className="bg-muted/30 rounded-lg p-3 space-y-2">
                            ...
                            {evt.delivered_to.length > 0 && (
                              <div className="text-xs text-muted-foreground">
                                {t("nerve.text17")}: {evt.delivered_to.join(", ")}
                              </div>
                            )}
                          </div>
                        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <h3 className="font-semibold mb-4 flex items-center gap-2">
              <Plus className="w-4 h-4" /> {t("nerve.add")}
            </h3>
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold">{t("nerve.text21")} ({subscriptions.length})</h3>
                  ...
                  <div key={sub.id} className="flex items-center gap-3 p-3 bg-muted/20 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-muted-foreground mt-1">
                        {t("nerve.text23")} {sub.delivery_count} {t("nerve.text24")}
                        {sub.last_delivery && ` · {t("nerve.text25")}: ${formatTime(sub.last_delivery)}`}
      ...
      {activeTab === "nodes" && (
        <div className="space-y-6">
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <h3 className="font-semibold mb-4 flex items-center gap-2">
              <Plus className="w-4 h-4" /> {t("nerve.text26")}
            </h3>
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold">{t("nerve.list")} ({nodes.length})</h3>
                ...
                {nodes.map(node => (
                  <div key={node.node_id} className="flex items-center gap-3 p-3 bg-muted/20 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-muted-foreground mt-1">
                        {t("nerve.text29")}: {formatTime(node.registered_at)}
                        {node.last_heartbeat && ` · {t("nerve.text30")}: ${formatTime(node.last_heartbeat)}`}
                      </div>
                    </div>
                    <button
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder={t("nerve.text33")}
                className="w-full bg-card border border-border rounded-lg px-3 py-2 text-sm"
              />
            </div>
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">{t("nerve.text34")})</label>
              <textarea
                value={pubData}
            ...
            >
              ...
              {pubLoading ? t("nerve.text35") : t("nerve.text5")}
            </button>
          </div>
            ...
            <div className="min-h-[300px] bg-card border border-border rounded-xl p-4">
              {pubResult ? (
                ...
                <div className="flex flex-col items-center justify-center h-full text-muted-foreground/50">
                  ...
                  <p className="text-smt("nerve.text38"){t("nerve.text5")}"</p>
                </div>
              )}
            </div>
          </div>
