...
const TYPE_LABELS: Record<string, string> = {
  tick: t('pulse.heartbeat2'),
  interval: t('pulse.interval2'),
  cron: t('pulse.schedule1'),
  once: t('pulse.oneTime'),
};
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCard icon={Heart} label={t("pulse.signals")} value={String(stats.total_signals)}
              sub={`${stats.by_status.active || 0} ${t("pulse.active")}`} color="text-red-500" bg="bg-red-500/10" />
            ...
            <StatCard icon={Activity} label={t("pulse.avgDrift")} value={formatMs(stats.precision.avg_drift_ms)}
              sub={`${stats.precision.sample_size} ${t("pulse.samples")}`} color="text-blue-500" bg="bg-blue-500/10" />
            ...
            <div className="rounded-xl border border-border p-4 space-y-3">
              ...
              <div className="grid grid-cols-5 gap-3">
                ...
                <select value={newType} onChange={(e) => setNewType(e.target.value)}
                  ...
                  <option value="interval">{t('pulse.interval3')}</option>
                  <option value="tick">{t('pulse.heartbeat3')}</option>
                  <option value="once">{t('pulse.oneTime1')}</option>
                </select>
                <input value={newInterval} onChange={(e) => setNewInterval(e.target.value)}
                  placeholder={t('pulse.interval4')}
                  className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-red-500/20" />
                <input value={newMaxFires} onChange={(e) => setNewMaxFires(e.target.value)}
                  placeholder={t('pulse.maxnonetimes')}
                  className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-red-500/20" />
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        {s.status === "active" ? (
                          ...
                          <button onClick={() => handleResume(s.signal_id)}
                            ...
                            title={t('pulse.restore1')}>
                            <Play size={14} />
                          </button>
                        ) : null}
                        <button onClick={() => handleDelete(s.signal_id)}
                          className="rounded-md p-1.5 text-muted-foreground hover:text-red-500 hover:bg-red-500/10 transition-colors"
                          title={t('pulse.delete5')}>
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/50">
                      <th className="px-4 py-2 text-left text-xs font-medium text-muted-foreground">{t('pulse.t32878')}</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-muted-foreground">{t('pulse.time6')}</th>
                      <th className="px-4 py-2 text-right text-xs font-medium text-muted-foreground">{t('pulse.t30209')}</th>
                      <th className="px-4 py-2 text-right text-xs font-medium text-muted-foreground">{t('pulse.latency2')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {ticks.map((t) => (
