...
function formatTime(iso: string): string {
  ...
  if (diff < 60000) return t("soma.text1");
  if (diff < 3600000) return `${Math.floor(diff / 60000)}{t("soma.text2")}`;
  if (diff < 86400000) return `${Math.floor(diff / 3600000)}{t("soma.text3")}`;
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-border px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold">{t("soma.text4")}</h1>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {health && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("soma.text7")}</span>
              <p className="text-2xl font-bold">{health.registry?.total || 0}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("soma.text8")}</span>
              <p className="text-2xl font-bold text-emerald-500">{health.registry?.online || 0}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("soma.text9")}</span>
              <p className="text-2xl font-bold text-red-500">{health.registry?.offline || 0}</p>
            </div>
            <div className="rounded-xl border border-border bg-card p-4">
              <span className="text-xs text-muted-foreground">{t("soma.text10")}</span>
              <p className="text-xs font-mono mt-1">
                {Object.entries(health.registry?.by_type || {}).map(([k, v]) => `${k}:${v}`).join(" · ") || t("soma.text11")}
              </p>
            </div>
          </div>
        )}
        ...
        {showCapabilities && capabilities && (
          <div className="rounded-xl border border-border bg-card p-5 space-y-4">
            <h3 className="text-sm font-semibold flex items-center gap-2">
              ...
              {t("soma.text12")} — {capabilities.platform} v{capabilities.version}
            </h3>
            ...
            <div className="border-t border-border pt-3">
              <h4 className="text-xs text-muted-foreground mb-2">API {t("soma.text13")}</h4>
        ...
        <div className="rounded-xl border border-dashed border-cyan-500/30 bg-cyan-500/5 p-5">
          <h3 className="text-sm font-semibold text-cyan-600 mb-2">🚀 {t("soma.text14")}</h3>
          <div className="text-xs text-muted-foreground space-y-2">
            <p>{t("soma.text15")}：</p>
            <div className="font-mono bg-background rounded p-3 text-[11px] space-y-1">
              <p><span className="text-cyan-500">1.</span> POST /api/soma/register — {t("soma.text16")}</p>
              <p><span className="text-cyan-500">2.</span> POST /api/soma/heartbeat — {t("soma.send")}</p>
              <p><span className="text-cyan-500">3.</span> POST /api/soma/push — {t("soma.text17")}</p>
              <p className="pt-1 text-muted-foreground"><span className="text-cyan-500">{t("soma.text18")}:</span> GET /api/soma/capabilities — {t("soma.auto")}</p>
            </div>
          </div>
        </div>
          ...
          <div className="w-80 space-y-3">
            {components.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                ...
                <p className="text-sm">{t("soma.empty")}</p>
                <p className="text-xs mt-1"{t("soma.text19")}t("soma.text6")或让外部组件自行{t("soma.text44")}</p>
              </div>
            ) : components.map((c) => (
              <div key={c.component_id}
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={cn("text-xs font-medium", STATUS_COLORS[c.status] || "text-muted-foreground")}>
                    {c.status === "online" ? t("soma.text21") : c.status === "offline" ? t("soma.text22") : c.status}
                  </span>
                </div>
                <div className="text-xs text-muted-foreground font-mono">{c.component_id}</div>
                <div className="flex gap-3 mt-2 text-xs text-muted-foreground">
                  <span>v{c.version}</span>
                  <span>{t("soma.text23")}: {c.data_push_count}</span>
                  {c.error_count > 0 && <span className="text-red-500">{t("soma.text24")}: {c.error_count}</span>}
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleUnregister(selected.component_id)}
                    ...
                    <Trash2 size={12} /> {t("soma.text26")}
                  </button>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><span className="text-muted-foreground">{t("soma.text27")}:</span> {selected.component_type}</div>
                <div><span className="text-muted-foreground">{t("soma.text28")}:</span> v{selected.version}</div>
                <div><span className="text-muted-foreground">{t("soma.time")}:</span> {formatTime(selected.registered_at)}</div>
                <div><span className="text-muted-foreground">{t("soma.text29")}:</span> {formatTime(selected.last_heartbeat)}</div>
                <div><span className="text-muted-foreground">{t("soma.text30")}:</span> {selected.data_push_count} {t("soma.text31")}</div>
                <div><span className="text-muted-foreground">{t("soma.text32")}:</span> {selected.error_count}</div>
              </div>

              {/* Capabilities */}
              {selected.capabilities.length > 0 && (
                <div>
                  <h4 className="text-xs text-muted-foreground mb-2">{t("soma.tags")}</h4>
              ...
              {selected.last_error && (
                <div className="rounded-lg border border-red-500/30 bg-red-500/5 p-3">
                  <div className="flex items-center gap-2 text-xs text-red-500">
                    ...
                    <span className="font-medium">{t("soma.text33")}</span>
                  </div>
        ...
        {showRegister && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 space-y-4">
              <h3 className="font-semibold">{t("soma.text34")}</h3>
              <input value={regId} onChange={(e) => setRegId(e.target.value)}
                placeholder={t("soma.text35")} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
              <input value={regName} onChange={(e) => setRegName(e.target.value)}
                placeholder={t("soma.name")} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
              <select value={regType} onChange={(e) => setRegType(e.target.value)}
                className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm">
                <option value="collector">{t("soma.text36")})</option>
                <option value="processor">{t("soma.text37")})</option>
                <option value="connector">{t("soma.text38")})</option>
                <option value="agent">Agent</option>
                <option value="custom">{t("soma.text39")}</option>
              </select>
              <input value={regVersion} onChange={(e) => setRegVersion(e.target.value)}
                placeholder={t("soma.text40")} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
              <input value={regCaps} onChange={(e) => setRegCaps(e.target.value)}
                placeholder={t("soma.text41")} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm" />
              <div className="flex justify-end gap-2">
                <button onClick={() => setShowRegister(false)}
                  className="rounded-lg border border-border px-4 py-2 text-sm hover:bg-muted">{t("soma.text42")}</button>
                <button onClick={handleRegister} disabled={loading || !regId.trim() || !regName.trim()}
                  ...
                  {loading ? t("soma.text43") : t("soma.text44")}
                </button>
              </div>
            </div>
          </div>
