...
export function VisionClient() {
  ...
  const [barTitle, setBarTitle] = useState(t("vision.defaultBarTitle") || "季度销售额");
  const [barColor, setBarColor] = useState("#e11d48");

  // Line chart form
  const [lineX, setLineX] = useState(t("vision.defaultLineX") || "1月,2月,3月,4月,5月,6月");
  const [lineSeries, setLineSeries] = useState(t("vision.defaultLineSeries") || "产品A:100,120,140,160,180,200\n产品B:80,90,110,130,120,150");
  const [lineTitle, setLineTitle] = useState(t("vision.defaultLineTitle") || "趋势对比");
  ...
  const [pieTitle, setPieTitle] = useState(t("vision.defaultPieTitle") || "语言使用分布");
  ...
  const [scatterTitle, setScatterTitle] = useState(t("vision.defaultScatterTitle") || "散点图");

  // Mind map form
  const [mindmapData, setMindmapData] = useState(JSON.stringify({
    label: t("vision.t64183") || "AI工程", children: [
      { label: "Prompt Engineering" },
      { label: "RAG", children: [{ label: t("vision.t60378") || "向量检索" }, { label: t("vision.t37406") || "混合召回" }] },
      { label: "Agent", children: [{ label: t("vision.t60786") || "工具调用" }, { label: t("vision.multiAgentCollab") || "多Agent协作" }] },
      { label: "Fine-tuning" },
    ],
  }, null, 2));
  const [mindmapTitle, setMindmapTitle] = useState(t("vision.defaultMindmapTitle") || "AI工程知识图谱");
  ...
  const handleGenerateMindmap = async () => {
    ...
    try {
      let root;
      try { root = JSON.parse(mindmapData); } catch { setError(t("vision.jsonError") || "JSON格式错误"); setLoading(false); return; }
