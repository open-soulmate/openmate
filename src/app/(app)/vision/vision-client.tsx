...
export function VisionClient() {
  ...
  const [barTitle, setBarTitle] = useState(t('vision.t13651'));
  const [barColor, setBarColor] = useState("#e11d48");

  // Line chart form
  const [lineX, setLineX] = useState(t('vision.t60525'));
  const [lineSeries, setLineSeries] = useState(t('vision.t19587'));
  const [lineTitle, setLineTitle] = useState(t('vision.t38378'));
  ...
  const [pieTitle, setPieTitle] = useState(t('vision.t36536'));
  ...
  const [scatterTitle, setScatterTitle] = useState(t('vision.scatterChart'));

  // Mind map form
  const [mindmapData, setMindmapData] = useState(JSON.stringify({
    label: t('vision.t64183'), children: [
      { label: "Prompt Engineering" },
      { label: "RAG", children: [{ label: t('vision.t60378') }, { label: t('vision.t37406') }] },
      { label: "Agent", children: [{ label: t('vision.t60786') }, { label: t('vision.multiAgentCollab') }] },
      { label: "Fine-tuning" },
    ],
  }, null, 2));
  const [mindmapTitle, setMindmapTitle] = useState(t('vision.t93963'));
  ...
  const handleGenerateMindmap = async () => {
    ...
    try {
      let root;
      try { root = JSON.parse(mindmapData); } catch { setError(t('vision.t84456')); setLoading(false); return; }
              ...
              <div className="rounded-xl border border-border bg-card p-5 space-y-3">
                ...
                {chartType === "line" && (
                  <>
                    ...
                    <Field label={`${t("vision.xlabel")} (${t("mind.chars")})`} value={lineX} onChange={setLineX} placeholder=t('vision.t80047') />
                    <div>
