      ...
      <div className="shrink-0 px-3 lg:px-6 pt-6 pb-4">
        <div className="flex items-center gap-2 lg:gap-3 mb-1">
          ...
          <div>
            ...
            <p className="text-xs lg:text-sm text-muted-foreground">{t("will.subtitle")}</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 lg:px-6 pb-6">
        ...
        {activeTab === "overview" && (
          <div className="space-y-3 lg:space-y-6">
            ...
            {workflows.length > 0 && (
              <div>
                <h3 className="text-xs lg:text-sm font-semibold text-foreground mb-3">{t("will.recentWorkflows")}</h3>
                <div className="space-y-2">
                  {workflows.slice(0, 5).map(wf => (
                    <div key={wf.id} className="flex items-center gap-2 lg:gap-3 p-3 rounded-lg border border-border bg-card">
                      <Workflow className="w-4 h-4 text-indigo-400" />
                      <div className="flex-1 min-w-0">
                        <div className="text-xs lg:text-sm font-medium text-foreground truncate">{wf.name}</div>
        ...
        {activeTab === "workflows" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xs lg:text-sm font-semibold text-foreground">{t("will.workflowManage")}</h3>
                ...
                <div className="grid grid-cols-2 gap-2 lg:gap-3">
                  <div>
                    ...
                    <input value={createForm.name} onChange={e => setCreateForm({ ...createForm, name: e.target.value })}
                      placeholder={t("will.createWorkflow")} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-xs lg:text-sm" />
                  </div>
                  <div>
                  ...
                  <div className="col-span-2">
                    ...
                    <input value={createForm.description} onChange={e => setCreateForm({ ...createForm, description: e.target.value })}
                      placeholder={t("will.descPlaceholder")} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-xs lg:text-sm" />
                  </div>
                </div>
                <div className="flex gap-2 justify-end">
                  <button onClick={() => setShowCreate(false)} className="px-3 py-1.5 rounded-lg border border-border text-xs lg:text-sm text-muted-foreground hover:text-foreground">{t("will.cancel")}</button>
                  <button onClick={handleCreate} disabled={!createForm.name}
                    className="px-4 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs lg:text-sm font-medium hover:opacity-90 disabled:opacity-40">{t("will.create")}</button>
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2 lg:gap-3">
                        ...
                        <div>
                          <div className="text-xs lg:text-sm font-semibold text-foreground">{wf.name}</div>
              ...
              {workflows.length === 0 && (
                <div className="text-center py-12 text-muted-foreground text-xs lg:text-sm">
        ...
        {activeTab === "executions" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xs lg:text-sm font-semibold text-foreground">{t("will.executions")}</h3>
              <button onClick={refresh} className="p-1.5 rounded-md hover:bg-accent text-muted-foreground hover:text-foreground">
                <RefreshCw className="w-4 h-4" />
              </button>
            </div>
            ...
            {selectedExec && (
              <div className="p-4 rounded-xl border border-border bg-card space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs lg:text-sm font-semibold text-foreground">{t("will.execDetail")}: {selectedExec.id.slice(0, 8)}</h4>
            ...
            <div className="space-y-2">
              {executions.map(ex => (
                <div key={ex.id} onClick={() => setSelectedExec(ex)}
                  className="flex items-center gap-2 lg:gap-3 p-3 rounded-lg border border-border bg-card cursor-pointer hover:border-primary/30 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs lg:text-sm font-medium text-foreground truncate">{ex.workflow_name || ex.workflow_id}</div>
                    <div className="text-xs text-muted-foreground">
