...
export function WorkflowClient() {
  ...
  const handleExecute = async (wf: WorkflowItem) => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(t('workflow.t63722', { name: wf.name }), "success");
        fetchWorkflows();
      } else {
        const data = await res.json();
        showToast(data.detail || t('common.executeFailed'), "error");
      }
    } catch {
  ...
  const handleTogglePause = async (wf: WorkflowItem) => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(t('workflow.t27509', { name: wf.name, newStatuspaused: newStatus === "paused" ? t('common.paused') : t('workflow.t50282') }), "success");
        fetchWorkflows();
      }
    } catch {
  ...
  const handleDelete = async (wf: WorkflowItem) => {
    try {
      ...
      if (res.ok) {
        showToast(t('workflow.t76266', { name: wf.name }), "success");
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        showToast(t('workflow.t63915', { newName: newName }), "success");
        setShowCreate(false);
        setNewName("");
        setNewDesc("");
        ...
        const data = await res.json();
        showToast(data.detail || t('common.createFailed'), "error");
      }
    } catch {
      showToast(t('common.networkError'), "error");
    }
        ...
        {showCreate && (
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-5 space-y-4">
            <h3 className="text-sm font-medium flex items-center gap-2">
              <Plus size={14} />  {t('workflow.t96212')}
            <h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <input value={newName} onChange={(e) => setNewName(e.target.value)}
                placeholder=t('workflow.t61850') className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30" />
              <input value={newDesc} onChange={(e) => setNewDesc(e.target.value)}
                placeholder=t('marrow.description') className="rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30" />
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={handleCreate} className="rounded-lg bg-primary px-4 py-2 text-sm text-primary-foreground hover:bg-primary/90">{t('common.create')}<button>
            </div>
          </div>
        )}
                    ...
                    <div className="flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                      ...
                      {(wf.status === "active" || wf.status === "paused") && (
                        <button onClick={() => handleTogglePause(wf)} disabled={!!busy}
                          className="flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:bg-accent hover:text-foreground"
                          title={wf.status === "paused" ? t('marrow.restore') : t('common.pause')}>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-xl">
            <h3 className="text-sm font-medium mb-2">{t('workflow.t72595')}<h3>
            <p className="text-xs text-muted-foreground mb-4">
              {t('workflow.t16460')}&quot;{deleteTarget.name}&quot;？{t('workflow.t19254')}            </p>
